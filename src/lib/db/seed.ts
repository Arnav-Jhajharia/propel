...
export async function seedDatabase() {
  ...
  try {
    ...
    const sampleAppointments = await db.insert(appointments).values([
      {
        userId: defaultUser[0].id,
        clientId: sampleClients[0].id,
        propertyId: sampleProperties[0].id,
        title: 'Property Viewing - Modern 2BR Condo',
        eventType: 'property_viewing',
        status: 'scheduled',
        startTime: new Date(today.getTime() + 10 * 60 * 60 * 1000).toISOString(), // 10:00 AM today
        endTime: new Date(today.getTime() + 11 * 60 * 60 * 1000).toISOString(), // 11:00 AM today
        timezone: 'Asia/Singapore',
        location: '12 Orchard Blvd, Singapore',
        notes: 'First viewing with Ava Tan',
        inviteeName: sampleClients[0].name,
        inviteeEmail: sampleClients[0].email,
        inviteePhone: sampleClients[0].phone,
      },
      {
        userId: defaultUser[0].id,
        clientId: sampleClients[2].id,
        propertyId: sampleProperties[1].id,
        title: 'Property Viewing - 3BR Tiong Bahru',
        eventType: 'property_viewing',
        status: 'scheduled',
        startTime: new Date(today.getTime() + 14 * 60 * 60 * 1000).toISOString(), // 2:00 PM today
        endTime: new Date(today.getTime() + 15 * 60 * 60 * 1000).toISOString(), // 3:00 PM today
        timezone: 'Asia/Singapore',
        location: '8 Kim Tian Rd, Singapore',
        notes: 'Family viewing with Priya Sharma',
        inviteeName: sampleClients[2].name,
        inviteeEmail: sampleClients[2].email,
        inviteePhone: sampleClients[2].phone,
      },
      {
        userId: defaultUser[0].id,
        clientId: sampleClients[1].id,
        propertyId: sampleProperties[0].id,
        title: 'Follow-up Call - Pet Policy Discussion',
        eventType: 'phone_call',
        status: 'scheduled',
        startTime: new Date(today.getTime() + 16 * 60 * 60 * 1000).toISOString(), // 4:00 PM today
        endTime: new Date(today.getTime() + 16.5 * 60 * 60 * 1000).toISOString(), // 4:30 PM today
        timezone: 'Asia/Singapore',
        notes: 'Discuss landlord pet policy with Marcus Lee',
        inviteeName: sampleClients[1].name,
        inviteeEmail: sampleClients[1].email,
        inviteePhone: sampleClients[1].phone,
      },
      {
        userId: defaultUser[0].id,
        title: 'Team Meeting - Weekly Property Review',
        eventType: 'meeting',
        status: 'scheduled',
        startTime: new Date(today.getTime() + 17 * 60 * 60 * 1000).toISOString(), // 5:00 PM today
        endTime: new Date(today.getTime() + 18 * 60 * 60 * 1000).toISOString(), // 6:00 PM today
        timezone: 'Asia/Singapore',
        location: 'Office Conference Room',
        notes: 'Weekly review of property listings and pipeline',
      },
    ]).returning();
